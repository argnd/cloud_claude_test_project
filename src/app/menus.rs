...
impl Overlay {
    ...
    pub fn show(
        ...
        match self {
            Overlay::Pause(menu) => {
                menu.show(painter, gfx, screen, input, game, audio, settings, time)
            }
            Overlay::Shop(shop) => shop.show(painter, gfx, screen, input, game, audio, time),
            Overlay::Service { kind, list } => {
                service(painter, gfx, screen, input, game, audio, *kind, list, time)
            }
            Overlay::Confirm { text, yes, list } => {
                ...
                let r = widgets::list(
                    ...
                    gfx,
                    Rect::from_min_size(
                        rect.left_top() + vec2(20.0, 100.0),
                        vec2(rect.width() - 40.0, 80.0),
                    ),
                    &rows,
                    list,
                    input,
            ...
            Overlay::FloorSelect(list) => {
                ...
                gfx::text(
                    painter,
                    rect.center_top() + vec2(0.0, 62.0),
                    Align2::CENTER_CENTER,
                    "The waystones remember every floor you have reached.",
                ...
                let r = widgets::list(
                    ...
                    gfx,
                    Rect::from_min_max(
                        rect.left_top() + vec2(20.0, 90.0),
                        rect.right_bottom() - vec2(20.0, 16.0),
                    ),
                    &rows,
                    list,
                    input,
            ...
            Overlay::Saves { saving, list } => {
                saves(painter, gfx, screen, input, audio, *saving, list, time)
            }
        }
    }
}
...
fn saves(
    ...
    let rows: Vec<Row> = (0..save::SLOTS)
        .map(|s| {
            ...
            Row::new(label)
                .right(desc.unwrap_or_else(|| "— empty —".into()))
                .enabled(enabled)
        })
        .collect();
    let r = widgets::list(
...
fn service(
    ...
    let (title, rows): (&str, Vec<Row>) = match kind {
        Shop::Inn => (
            "The Lantern & Lark",
            vec![
                Row::new("Rest the night").right("free"),
                Row::new("Buy provisions"),
                Row::new("Save the game"),
                Row::new("Leave"),
            ],
        ),
        _ => (
            "Temple of the Flame",
            vec![
                Row::new("Pray").right("heals and revives"),
                Row::new("Buy blessings"),
                Row::new("Save the game"),
                Row::new("Leave"),
            ],
        ),
...
impl ShopView {
    ...
    fn show(
        ...
        let rect = Rect::from_center_size(
            screen.center(),
            vec2(
                1000.0f32.min(screen.width() - 40.0),
                620.0f32.min(screen.height() - 40.0),
            ),
        );
        gfx::panel(painter, rect);
        let (title, keeper) = match self.shop {
            Shop::Smith => (
                "Dagna's Forge",
                "\"Steel doesn't care who you are. Neither do I. Pay up.\"",
            ),
            Shop::Apothecary => (
                "Fen's Apothecary",
                "\"Everything's labelled! Mostly! Please don't drink the blue one twice.\"",
            ),
            Shop::Inn => (
                "Bess's Pantry",
                "\"Nobody goes down there on an empty stomach. Not on my watch.\"",
            ),
            Shop::Temple => (
                "Temple Offerings",
                "\"May the Flame keep you. And may these help it along.\"",
            ),
        };
        gfx::text(
            painter,
        ...
        if widgets::tabs(painter, tab_rect, &["Buy", "Sell"], &mut mode, input)
            || input.left
            || input.right
        {
        ...
        let items: Vec<(ItemId, u32)> = if self.selling {
            game.inventory
                .list()
                .into_iter()
                .filter(|(i, _)| i.def().price > 0 && !matches!(i.def().kind, ItemKind::Quest))
                .collect()
        } else {
            game.stock(self.shop)
                .into_iter()
                .map(|i| (i, game.inventory.count(i)))
                .collect()
        };
        let rows: Vec<Row> = items
            ...
            .map(|&(i, have)| {
                ...
                let label = if have > 0 {
                    format!("{}  (have {have})", d.name)
                } else {
                    d.name.to_string()
                };
                Row::new(label)
                    .right(format!("{price} g"))
                    .icon(d.sprite)
                    .enabled(enabled)
            })
            .collect();
        let list_rect = Rect::from_min_max(
            rect.left_top() + vec2(20.0, 136.0),
            pos2(rect.left() + rect.width() * 0.52, rect.bottom() - 18.0),
        );
        let r = widgets::list(
            painter,
            gfx,
        ...
        if let Some(&(item, _)) = items.get(self.list.cursor) {
            let detail = Rect::from_min_max(
                pos2(list_rect.right() + 24.0, rect.top() + 136.0),
                rect.right_bottom() - vec2(24.0, 18.0),
            );
            item_details(painter, gfx, detail, game, item);
        }
        if let Some(i) = r.picked {
            let item = items[i].0;
            let result = if self.selling {
                game.sell(item).map(|g| format!("Sold for {g} gold."))
            } else {
                game.buy(item)
                    .map(|_| format!("Bought {}.", item.def().name))
            };
            match result {
                Ok(m) => {
        ...
        if let Some((m, t)) = &mut self.message {
            *t -= input.dt;
            gfx::text(
                painter,
                pos2(rect.center().x, rect.bottom() - 20.0),
...
fn item_details(painter: &egui::Painter, gfx: &Gfx, rect: Rect, game: &Game, item: ItemId) {
    ...
    gfx::text(
        painter,
        pos2(icon.right() + 14.0, rect.top() + 8.0),
        Align2::LEFT_TOP,
        d.name,
        gfx::heading_font(22.0),
        GOLD,
    ...
    gfx::text(
        painter,
        pos2(icon.right() + 14.0, rect.top() + 40.0),
        Align2::LEFT_TOP,
        kind,
    ...
    if let Some(slot) = d.slot() {
        for h in &game.party {
            ...
            let text = if !can {
                ...
                let delta = |v: i32, n: &str| {
                    if v == 0 {
                        String::new()
                    } else {
                        format!("{n} {}{v}  ", if v > 0 { "+" } else { "" })
                    }
                };
                let t = format!(
                    "{}{}{}{}{}{}{}",
                ...
                format!(
                    "{}: {}",
                    h.name(),
                    if t.is_empty() { "no change".into() } else { t }
                )
            };
            let colour = if !can {
            ...
            gfx.draw(
                painter,
                h.def().sprite,
                Rect::from_min_size(pos2(rect.left(), y - 4.0), vec2(28.0, 28.0)),
                if can {
                    Color32::WHITE
                } else {
...
const TABS: [&str; 7] = [
    "Party", "Equip", "Items", "Skills", "Quests", "Journal", "System",
];
...
impl PauseMenu {
    ...
    fn show(
        ...
        let rect = Rect::from_center_size(
            screen.center(),
            vec2(
                (screen.width() - 60.0).min(1180.0),
                (screen.height() - 60.0).min(720.0),
            ),
        );
        gfx::panel(painter, rect);
        let tab_rect = Rect::from_min_size(
            rect.left_top() + vec2(20.0, 16.0),
            vec2(rect.width() - 40.0, 38.0),
        );
        ...
        let body = Rect::from_min_max(
            rect.left_top() + vec2(20.0, 70.0),
            rect.right_bottom() - vec2(20.0, 20.0),
        );
        ...
        if let Some((m, t)) = &mut self.message {
            *t -= input.dt;
            let r = Rect::from_center_size(
                ...
                vec2(m.len() as f32 * 10.0 + 60.0, 36.0),
            );
    ...
    fn party(
        ...
        gfx::text(
            painter,
            pos2(x, right.top() + 50.0),
            Align2::LEFT_TOP,
            &format!("{} · Level {}", h.def().title, h.level),
            gfx::heading_font(20.0),
            PARCHMENT,
        );
        gfx::wrapped(
            painter,
            pos2(x, right.top() + 82.0),
            right.right() - x,
            h.def().blurb,
        ...
        gfx::bar(
            painter,
            Rect::from_min_size(pos2(x, right.top() + 124.0), vec2(bw, 14.0)),
            h.hp.max(0) as f32 / h.max_hp() as f32,
            gfx::HP_RED,
        );
        gfx::text(
            painter,
            pos2(x + bw + 10.0, right.top() + 131.0),
            Align2::LEFT_CENTER,
            &format!("HP {} / {}", h.hp.max(0), h.max_hp()),
            gfx::body_font(18.0),
            PARCHMENT,
        );
        gfx::bar(
            painter,
            Rect::from_min_size(pos2(x, right.top() + 146.0), vec2(bw, 10.0)),
            h.mp as f32 / h.max_mp().max(1) as f32,
            gfx::MP_BLUE,
        );
        gfx::text(
            painter,
            pos2(x + bw + 10.0, right.top() + 151.0),
            Align2::LEFT_CENTER,
            &format!("MP {} / {}", h.mp, h.max_mp()),
            gfx::body_font(18.0),
            PARCHMENT,
        );
        ...
        gfx::bar(
            painter,
            Rect::from_min_size(pos2(x, right.top() + 164.0), vec2(bw, 6.0)),
            xp_frac,
            GOLD,
        );
        gfx::text(
            painter,
            pos2(x + bw + 10.0, right.top() + 167.0),
            Align2::LEFT_CENTER,
            &format!("{} XP to next", h.xp_to_next().saturating_sub(h.xp)),
            gfx::body_font(15.0),
            gfx::DIM,
        );
        let s = h.stats();
        ...
        .enumerate()
        {
            ...
            if let Some(i) = item {
                gfx.draw(
                    painter,
                    i.def().sprite,
                    Rect::from_min_size(pos2(gx + 110.0, y - 6.0), vec2(32.0, 32.0)),
                    Color32::WHITE,
                );
            }
            gfx::text(
                ...
                pos2(gx + 148.0, y),
                Align2::LEFT_TOP,
                item.map(|i| i.def().name).unwrap_or("—"),
                gfx::body_font(20.0),
                PARCHMENT,
            );
        }
        ...
        gfx::text(
            painter,
            pos2(right.left(), body.bottom() - 70.0),
            Align2::LEFT_TOP,
            "Skills",
            gfx::heading_font(16.0),
            gfx::DIM,
        ...
        gfx::wrapped(
            painter,
            pos2(right.left(), body.bottom() - 46.0),
            right.width(),
            &skills.join(" · "),
            gfx::body_font(18.0),
            PARCHMENT,
    ...
    fn equip(
        ...
        let b = Rect::from_min_size(
            pos2(a.right() + 15.0, body.top()),
            vec2(col, body.height() * 0.45),
        );
        ...
        let rows: Vec<Row> = options
            ...
            .map(|o| match o {
                None => Row::new("(remove)"),
                Some(i) => Row::new(i.def().name).icon(i.def().sprite).right(
                    stats_text(i.def().stats)
                        .split("   ")
                        .next()
                        .unwrap_or("")
                        .to_string(),
        ...
        if self.stage == 2 {
            ...
            if let Some(Some(i)) = options.get(self.c.cursor) {
                let detail = Rect::from_min_max(
                    pos2(b.left(), b.bottom() + 20.0),
                    pos2(b.right(), body.bottom()),
                );
            ...
            gfx::text(
                ...
                detail,
                Align2::LEFT_TOP,
                &format!("ATK {}   DEF {}   MAG {}", s.atk, s.def, s.mag),
                gfx::body_font(20.0),
                PARCHMENT,
            );
            gfx::text(
                painter,
                detail + vec2(0.0, 30.0),
                Align2::LEFT_TOP,
                &format!("RES {}   SPD {}   HP {}   MP {}", s.res, s.spd, s.hp, s.mp),
                gfx::body_font(20.0),
                PARCHMENT,
            );
        }
    ...
    fn items(
        ...
        if self.stage == 0 {
            ...
            let rows: Vec<Row> = game
                ...
                .iter()
                .map(|h| {
                    Row::new(h.name()).icon(h.def().sprite).right(format!(
                        "HP {}/{}  MP {}/{}",
                        h.hp.max(0),
                        h.max_hp(),
                        h.mp,
            ...
            let r = widgets::list(
                painter,
                gfx,
                Rect::from_min_max(right.left_top() + vec2(0.0, 40.0), right.right_bottom()),
                &rows,
                &mut self.b,
                input,
                true,
    ...
    fn skills(
        ...
        let rows: Vec<Row> = skills
            ...
            .map(|s| {
                ...
                Row::new(d.name)
                    ...
                    .enabled(
                        d.usable_outside_battle()
                            && game.party[caster].mp >= d.mp
                            && game.party[caster].hp > 0,
                    )
            })
            .collect();
        let r = widgets::list(
        ...
        if self.stage == 2 {
            ...
            if let Some(t) = r.picked {
                ...
                let targets: Vec<usize> = if d.target == Target::AllAllies {
                    (0..game.party.len()).collect()
                } else {
                    vec![t]
                };
                let mut did = false;
    ...
    fn quests(
        ...
        gfx::text(
            painter,
            body.left_top() + vec2(0.0, 30.0),
            Align2::LEFT_TOP,
            game.objective(),
            gfx::italic_font(20.0),
            PARCHMENT,
        );
        let quests: Vec<QuestId> = QuestId::ALL
            .into_iter()
            .filter(|&q| game.quest_state(q) != QuestState::Hidden)
            .collect();
        let left = Rect::from_min_max(
            body.left_top() + vec2(0.0, 80.0),
            pos2(body.left() + body.width() * 0.45, body.bottom()),
        );
        ...
        if let Some(&q) = quests.get(self.a.cursor) {
            ...
            gfx::text(
                painter,
                right.left_top() + vec2(0.0, 32.0),
                Align2::LEFT_TOP,
                &format!("From {}", d.giver_name),
                gfx::italic_font(18.0),
                gfx::DIM,
            );
            ...
            if let crate::data::quests::Requirement::Items(item, n) = d.requirement
                && game.quest_state(q) != QuestState::Done
            {
                gfx::text(
                    painter,
                    pos2(right.left(), right.bottom() - 30.0),
                    Align2::LEFT_TOP,
                    &format!(
                        "{}: {} / {n}",
                        item.def().name,
                        game.inventory.count(item).min(n)
                    ),
                    gfx::body_font(20.0),
                    GOLD,
                );
    ...
    fn journal(
        ...
        let rows: Vec<Row> = entries
            .iter()
            .map(|(label, id, have)| {
                let icon = if id.starts_with("shard") {
                    crate::gfx::sprites::Sprite::MemoryShard
                } else {
                    crate::gfx::sprites::Sprite::NoteScroll
                };
        ...
        gfx::text(
            ...
            right.left_top(),
            Align2::LEFT_TOP,
            &format!("Memory Shards: {} / 12", shards.len()),
            gfx::heading_font(22.0),
        ...
        gfx::text(
            painter,
            right.left_top() + vec2(0.0, 170.0),
            Align2::LEFT_TOP,
            &format!("Ilsa's pages: {} / 5", journals.len()),
            gfx::heading_font(22.0),
            GOLD,
        );
        ...
        gfx::text(
            painter,
            right.left_top() + vec2(0.0, 220.0),
            Align2::LEFT_TOP,
            &format!("Weaknesses discovered: {known}"),
            gfx::heading_font(18.0),
            gfx::DIM,
        );
        gfx::text(
            painter,
            pos2(right.left(), body.bottom() - 30.0),
            Align2::LEFT_TOP,
            "Enter: read again",
            gfx::italic_font(17.0),
            gfx::DIM,
    ...
    fn system(
        ...
        let left = Rect::from_min_size(
            body.min + vec2(body.width() * 0.25, 20.0),
            vec2(body.width() * 0.5, body.height() - 20.0),
        );
...
pub fn settings_panel(
    ...
    let rows = vec![
        ...
        Row::new("Fullscreen").right(if settings.fullscreen {
            "<  On  >"
        } else {
            "<  Off  >"
        }),
    ...
    if delta != 0.0 || r.picked.is_some_and(|i| i < 5) {
        ...
        match list.cursor {
            ...
            2 => {
                ...
                let i = steps
                    .iter()
                    .position(|&s| s >= settings.text_speed - 1.0)
                    .unwrap_or(1) as i32;
